...
public class CurrentDayBuilder {
    ...
    private DailyEarningsDto getCurrent(List<DailyEarningsDto> days) {
        ...
        logger.log(Level.DEBUG, "Pattern of date in monthly list is " + days.stream().findFirst().orElse(DailyEarningsDto.builder().earnings("Still Empty").build()));
        logger.log(Level.DEBUG, "Size of days in monthly list is " + (days != null ? days.size() : null));
